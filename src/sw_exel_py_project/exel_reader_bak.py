...
def fifo_for_product(file_path: Path, product: str, year: int, sales_qty: float):
    allocations, remaining, check_year = [], sales_qty, year
    while remaining > 0 and check_year >= 2000:
        inbound = load_inbound_history(file_path, check_year, product)
        if inbound.empty: 
            check_year -= 1; continue
        alloc, remaining = fifo_allocate(remaining, inbound)
        allocations.extend(alloc); check_year -= 1
    if remaining > 0:
        raise ValueError(f"{product}: {sales_qty}kg 중 {remaining}kg 소진 불가 (입고 부족)")
    return allocations
