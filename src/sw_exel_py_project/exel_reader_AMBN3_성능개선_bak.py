...
def load_inventory_df(file_path_str: str, year: int) -> pd.DataFrame:
    """
    파일/연도 단위로 엑셀을 1회만 읽고, 필요한 파생 컬럼들을 미리 생성해 반환.
    생성 컬럼:
      - __product__
      - __event_date__ (판매/출고 기준일: '날 짜' 기본, NaT 시 '도착일')
      - __in_date__ (입고/수입 기준일: '도착일' 우선)
      - __sales_qty__ (납품/매출 수량 합)
      - __in_qty__ (입고/수입 수량 합)
      - __unit_price__ (입고/수입 단가 평균)
      - __exchange_rate__ (입고/수입 환율 평균)
    """
    ...
    date_col    = _pick_date_col(df, INVENTORY_COLUMN_MAP["date"])            # '날 짜'
    prod_col    = _pick_product_col(df, INVENTORY_COLUMN_MAP["product_name"])  # '품 명'
    ...
    df["__in_date__"]    = _make_inbound_date_series(df, date_col, arrival_col, year)

    # 판매수량 합산
    try:
        sales_qty_cols = _pick_sales_qty_cols(df)
        df["__sales_qty__"] = (
            df[sales_qty_cols]
    ...
    if price_cols:
        ...
        df["__unit_price__"] = pd.NA

    if rate_cols:
        df["__exchange_rate__"] = (
            df[rate_cols]
            .replace(r"[^\d\.\-]", "", regex=True)
            .apply(pd.to_numeric, errors="coerce")
            .mean(axis=1)
...
def load_inbound_history(file_path: Path, year: int, product: str, df: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    inbound = (
        inbound.query("product == @product and quantity > 0")
               .dropna(subset=["in_date"])  # 날짜 없는 로트 제외
               .copy()
    )
...
def _only_priced_lots(df: pd.DataFrame) -> pd.DataFrame:
    """단가/환율이 모두 유효한(숫자) 로트만 남긴다."""
    out = df.copy()
    out["unit_price"] = pd.to_numeric(out["unit_price"], errors="coerce")
    out["exchange_rate"] = pd.to_numeric(out["exchange_rate"], errors="coerce")
    return out[out["unit_price"].notna() & out["exchange_rate"].notna()].copy().reset_index(drop=True)
...
def sum_sales_in_range(file_path: Path, year: int, product: str,
    ...
    total = float(pd.to_numeric(df.loc[mask, "__sales_qty__"], errors="coerce").fillna(0.0).sum())
    return total
...
def apply_fifo_consumption(lots_df: pd.DataFrame, consume_qty: float):
    ...
    for i in range(len(lots)):
        if remaining <= 0:
            break
        # 인덱스가 0..N-1가 아닐 수 있으므로 label-safe 접근
        idx = lots.index[i]
...
def get_current_stock(file_path: Path, year: int, product: str, start: pd.Timestamp, end: pd.Timestamp,
                      df: pd.DataFrame | None = None) -> float:
...
def fifo_for_product(
    ...
    inbound_all = load_inbound_history(file_path, year, product, df=df)
    inbound_all = (
    ...
    start_snapshot = get_stock_snapshot_at_or_before(file_path, year, product, boundary, df=df)
    inbound_before_start = sum_inbound_until(file_path, year, product, boundary, df=df)
   ...
   # 5) 기간 배분 풀 = (선소진 후 pre 잔량) + (post_lots)
    ...
    # ✅ 가격 확정 로트만 사용 (단가/환율 NaN 로트는 기간배분에서 제외)
    if not period_pool.empty:
        ...
        # ★ 핵심 필터: 단가/환율이 없는 로트 제외
        period_pool = _only_priced_lots(period_pool)
...
def get_stock_snapshot_at_or_before(file_path: Path, year: int, product: str, asof: pd.Timestamp,
                                    df: pd.DataFrame | None = None) -> float:
...
def sum_inbound_until(file_path: Path, year: int, product: str, until: pd.Timestamp,
                      df: pd.DataFrame | None = None) -> float:
    ...
    mask2 = (df["__product__"] == product) & (df["__in_date__"] <= until)
    return float(pd.to_numeric(df.loc[mask2, "__in_qty__"], errors="coerce").fillna(0.0).sum())
# ======================
# 최종 실행
# ======================
...
def filter_and_fifo(result: dict):
    ...
    # 엑셀/파생 컬럼을 1회만 준비하여 전체 루프에서 재사용
    ...
    for _, row in sales_df.iterrows():
        ...
        current_stock = get_current_stock(file_path, year, product, start, end, df=df_all)  # 보고용

        print(f"DEBUG - FIFO 처리중: {product}, 판매수량={qty}, 현재고(보고)={current_stock}")
        allocations = fifo_for_product(file_path, product, year, qty, start=start, cutoff=end, current_stock=current_stock, df=df_all)
